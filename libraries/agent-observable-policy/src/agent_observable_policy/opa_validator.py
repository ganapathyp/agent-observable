"""OPA-based tool call validator (framework-agnostic).

Automatically tracks policy decisions using standardized metric names.
...
from .decision import DecisionType, DecisionResult, PolicyDecision
from .decision_logger import DecisionLogger
from .opa_embedded import EmbeddedOPA

# Optional imports for standardized metrics
try:
    from agent_observable_core.framework_detector import get_metric_standardizer
    HAS_METRIC_STANDARDIZER = True
except ImportError:
    HAS_METRIC_STANDARDIZER = False
...
class OPAToolValidator:
    ...
    def __init__(
        ...
        Args:
            ...
            embedded_opa: Optional EmbeddedOPA instance (created if not provided)
            decision_logger: Optional DecisionLogger instance (decisions won't be logged if None)
            package: Policy package name (default: "taskpilot.tool_calls")
        """
        ...
        if use_embedded:
            self.embedded_opa = embedded_opa or EmbeddedOPA()
            self._enabled = True
    ...
    async def validate_tool_call(
        ...
        try:
            ...
            if self.use_embedded and self.embedded_opa:
                ...
                if not requests:
                    return True, "requests library not available", False
            ...
            if self._decision_logger:
                ...
                await self._decision_logger.log_decision(decision)

            # Track policy metrics using standardized names
            if self.metrics_collector and HAS_METRIC_STANDARDIZER:
                try:
                    standardizer = get_metric_standardizer(service_name=self.service_name)
                    if not allowed:
                        self.metrics_collector.increment_counter(standardizer.policy_violations_total())
                except Exception:
                    pass
